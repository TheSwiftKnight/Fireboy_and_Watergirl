...
void PlayScene::ReadMap() {
	...
	for (int i = 0; i < MapHeight; i++) {
		for (int j = 0; j < MapWidth; j++) {
			const char token = mapData[i * MapWidth + j];
			if(token=='1'){
				...
				mapState[i][j] = TILE_BUTTON0;
				TileMapGroup->AddNewObject(new Engine::Image("play/button0.png", j * BlockSize, i * BlockSize, BlockSize, BlockSize));
			}
			else if(token == 'E'){
